...
Posterior samples were generated from a model fitted to an entirely hypothetical data set.
...
This is a state-space formulation, where there are explicit random processes for both the true state of each fish (alive or dead) and the observed state of each fish (seen or not seen) in each event. Both are Bernoulli random variables, with `z` representing the true state and `y` representing the observed state. It is assumed that fish cannot be detected if they are dead. Survival probability between two consecutive detection events for fish `i` is denoted by `phi[i]` and for this model is expressed as a logit-linear function of fish size (`FL[i]`, scaled and centered prior to model fitting), with random slopes and intercepts for each year. Detection probability for this model is assumed to vary among detection arrays, but is constant across years.
...
The `mcmc.list` object called `cjs` is from this model exactly, `cjs_no_rho` is from the same model, but with the `rho` parameter fixed at zero rather than estimating its value.

The nodes that were monitored include: the global survival coefficients (`B0` and `B1`), their year-specific random realizations (`b0` and `b1`), the covariance matrix modeling the inter-annual covariability (`SIG`), and the detection probability of each array (`p`). See:
