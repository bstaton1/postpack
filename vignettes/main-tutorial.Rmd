...
The R package `postpack` exists to make your job easier when it comes time to process the output of models fitted using Markov Chain Monte Carlo (MCMC) methods, as is commonly done in Bayesian inference. Although significant capabilities exist to interface from R with programs like JAGS, WinBUGS, OpenBUGS, and Stan, the functionality to easily process the output is lacking in my personal opinion. Output from these models is commonly stored in (or can easily be converted to) R objects of class `mcmc.list` (default methods supplied through the `coda` package, upon which `postpack` strongly depends), and working with these objects is not as straight-forward as one might desire. A practitioner needs to extract raw samples, summarize them, diagnose their convergence, and plot the summaries along with their measures of uncertainty. It would be ideal if consistent rules were used in how these actions were performed, all based around the master `mcmc.list`, and the functionality should be able to scale to seamlessly summarize the same output from multiple models to allow quick evaluation of the influence of different model assumptions (e.g., different priors or where some terms are fixed at zero). Perhaps most importantly, given the ever-increasing complexity of these models, these tasks should be made simple to perform for only desired nodes from the model.

# Working Example

For this tutorial, suppose you have fitted the following Cormack-Jolly-Seber mark-recapture model^[Readers interested in this kind of model are encouraged to refer to _Bayesian Population Analysis using WinBUGS: A Hierarchical Perspective_ by Marc Kéry and Michael Schuab.]:
...
You can extract one of these dimensional elements quickly by specifying the `kind` argument, e.g., notice that there are `r post_dim(cjs, "nodes")` saved nodes by running: `post_dim(cjs, "nodes")`.

The names of these nodes will be crucial in subsetting them, so you should be able to check them out quickly at any time. This is the purpose of the `get_p()` function:

```{r}
get_p(cjs)
```

But I thought we stored `r post_dim(cjs, "nodes")` nodes? This is because some of the nodes have multiple elements; you can access the indices monitored for each node as well:
...
With `post_summ()` you can summarize only the node(s) you wish by passing their name(s) to the `p` argument.

```{r}
post_summ(cjs, p = c("sig_B0", "sig_B1"))
```
...
```{r}
post_summ(cjs, p = c("sig_B0", "sig_B1"), ess = T, Rhat = T)
```

`"Rhat"` will always be rounded to three digits, and `"ess"` will always be rounded to an integer.

You can report other quantiles if desired and round them:

```{r}
b1_ests = post_summ(cjs, "b1", rnd = 3,
  p_summ = c(0.025, 0.25, 0.5, 0.75, 0.975)
)
```
...
By default, the output will be a `mcmc.list` which allows it to play nicely with the rest of the `postpack` functions (e.g., `get_p(b0_samps)`). However, performing plotting or calculation tasks on posterior samples might be easier if they were combined across chains and stored as a matrix (nodes as columns, rows as samples):
...
because all of the node names contain a `"B"`, they are matched and returned. So if you only wanted `sig_B0` and `sig_B1`, you could pass `p = "sig"` as an argument to the `postpack` function in question, to perform the action for only nodes that contain `"sig"`.

## `match_p()`

Pattern matching is driven by the `match_p()` function (which is essentially a wrapper for `stringr::str_detect()`). Every function in `postpack` that uses a `p` argument passes that argument to `match_p()` early on their calculations, so understanding the way it works is important. For example, say you want the random slope and intercept for year 2. Use `match_p()` to help you get the right regular expression code by checking what will be matched when you try to subset your output:
...
Because the square brackets (`"["` and `"]"`) are special characters in regular expressions, you would normally need to escape them (`"\\["` and `"\\]"`) to allow them to match as text. However, I built in functionality to force brackets to be escaped prior to pattern matching, and to ignore escapes if they are already inserted. Thus, you won't be able to use the (`"["` and `"]"`) for their default pattern matching behavior in regular expressions (e.g., match any letter: `"[:alpha:]"`), until an `auto_escape` argument is added (which I plan to in the future).
...
Which is what will happen when any elements supplied to `p` do not match one of the node names stored in the output. The wildcard is necessary to specify that some character (here a `"1"` or a `"2"`) falls between the `"b"` and the `"["` characters in your desired match. You can now pass `"b.[2]"` to `post_summ()` with confidence knowing that only the two coefficients you want will be returned:
...
Now that you have a basic understanding of how to use the `p` argument within `postpack` functions to be very specific about which output you want to obtain, its time to cover some other other functionality provided with `postpack`.
...
*  to plot the output in a new external device (the `ext_device` argument, the device will be OS specific, e.g., `quartz()` on a Mac or `windows()` on a PC - see `?StatonMisc::ext_device`), 
*  to save the output to a PDF graphics device (the `save` argument, which then requires that you enter the `file` argument, which is the file name of the PDF complete with the `".pdf"` extension). 
*  thin the chains by some percentage (at evenly spaced intervals) before drawing the trace plot - this can help manage the file size when generating many plots with many samples taken while still providing much of the same inference as the unthinned output. The argument `thin_percent = 0.8` is passed to `post_thin`, and would discard 80% of the samples prior to trace plotting.

The `dims` and `layout` arguments are set to `"auto"` by default - for adjustment of these settings, see `?diag_plots`. 

The more chains there are in the `mcmc.list` object passed to the `post` argument, the more colors there will be displayed.
...
```{r}
(SIG_ests = post_summ(cjs, "SIG", rnd = 2))
```
...
Currently, objects with dimensions between 2 and 10 are supported. Although this is a silly example, imagine you had arranged survival for each individual, detection occasion, and year as `phi[i,j,y]` - it would be really nice to be able to quickly put the summaries back in the dimensions of the model.
...
```{r, echo = F}
SIG_decomp = vcov_decomp(cjs, "SIG", progress = F)
```
...
2.)  Deciding on a name for each of your quantities,

3.)  Coercing the matrix to a `mcmc.list` object with consistent dimensions as the master list, and

4.)  Binding the derived list to the master list.
...
For step (3), you must assign each sample to an iteration number and chain, so that the corresponding samples can be matched with the jointly sampled quantities in the master output. This ensures consistency - as though the derived quantity was calculated within the model to begin with. Extract the appropriate identifiers using `id_mat()` from the master object and combine them by columns with the derived quantity:

```{r}
eb0_samps = cbind(id_mat(cjs), eb0_samps)
```

You can now complete step (3) by coercing the derived object to an `mcmc.list` with the `postpack` function `matrix2mcmclist()`:

```{r}
eb0_samps = matrix2mcmclist(eb0_samps)
```

Finally, for step (4) combine the samples using `post_bind()`:

```{r}
cjs = post_bind(post1 = cjs, post2 = eb0_samps)
```

If the two `mcmc.list` objects contain duplicate node names, the values from the list passed to the `post2` argument of `post_bind()` will have the suffix supplied under the argument `dup_id` (`"_p2"` by default).

In the future, the steps in this section may be wrapped into the default behavior of `post_bind()`, especially, since this is one of the few places where the whole function `id_mat()` is necessary.
...
From here, the world is wide open to you. Anything you would do with one of your `mcmc.list` objects, you can do with two (or any number really) through the use of the base `apply()` family of functions. For example, extract the dimensions of the saved output for each run:

```{r}
sapply(post_list, post_dim)
```

Notice the two are identical, except that the `"no_rho"` run has fewer nodes because we added some to the `cjs` object above. You can see this by applying `get_p()` separately to each model object:

```{r}
sapply(post_list, get_p)
```
...
```{r}
lapply(post_list, function(model) post_summ(model, "^B", rnd = 2))
```

Or verify that the detection efficiencies are not affected either:

```{r}
lapply(post_list, function(model) post_summ(model, "p", rnd = 2))
```
